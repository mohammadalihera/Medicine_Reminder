...
class GetMedicineController extends GetxController {
  ...
  String firstDayOfWeek = CacheService.instance.firstDayOfWeek.get('firstDayOfWeek') != null
  ...
  void getAllVitelFromDb(BuildContext context) async {
    ...
    for (int i = 0; i < allVitel.length; ++i) {
      List<String> dbDates = [];
      if (allVitel[i].date.contains(',') == true) {
        dbDates = allVitel[i].date.split(',');
      } else {
        dbDates.add(allVitel[i].date);
      }

      for (int j = 0; j < dbDates.length; ++j) {
        if (dbDates[j].isNotEmpty) {
          int dateInt = int.parse(dbDates[j]);
          DateTime mapDate = DateTime(DateTime.fromMillisecondsSinceEpoch(dateInt).year,
              DateTime.fromMillisecondsSinceEpoch(dateInt).month, DateTime.fromMillisecondsSinceEpoch(dateInt).day);
      ...
      DateTime alarm = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, DateTime.now().hour,
  ...
  void getNotification(BuildContext context) async {
    ...
    for (int i = 0; i < allVitel.length; ++i) {
      DateTime alarm = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, DateTime.now().hour,
          DateTime.now().minute + 2);
      await _notifications.showNotification(allVitel[i].name, allVitel[i].program.toString(), alarm.millisecond,
          allVitel[i].id, flutterLocalNotificationsPlugin);
    }
